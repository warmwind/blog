...
Claude Code 将一个能对你的代码进行推理的 model，与用于文件操作、搜索、执行和 Web 访问的[内置 tools](/en/how-claude-code-works#tools)结合在一起。内置 tools 覆盖了大多数编码任务。本指南聚焦扩展层：你可借此自定义 Claude 已知内容、连接外部服务并自动化工作流。

> 关于核心 agentic loop 的工作方式，参见 [How Claude Code works](/en/how-claude-code-works)。

**刚接触 Claude Code？** 先从项目约定的 [CLAUDE.md](/en/memory) 开始。其他扩展按需添加。

## 概览

扩展可插入 agentic loop 的不同位置：

- **[CLAUDE.md](/en/memory)**：添加持久上下文，Claude 每次会话都能看到
- **[Skills](/en/skills)**：添加可复用知识和可调用工作流
- **[MCP](/en/mcp)**：将 Claude 连接到外部服务与 tools
- **[Subagents](/en/sub-agents)**：在隔离上下文中运行独立 loop，并返回摘要
- **[Agent teams](/en/agent-teams)**：协调多个独立会话，支持共享任务与点对点消息
- **[Hooks](/en/hooks)**：完全在 loop 外运行，作为确定性脚本
- **[Plugins](/en/plugins)** 与 **[marketplaces](/en/plugin-marketplaces)**：用于打包与分发上述能力

[Skills](/en/skills) 是最灵活的扩展。skill 是包含知识、工作流或指令的 markdown 文件。你可以用 `/deploy` 这类命令调用 skill，或者让 Claude 在相关时自动加载。skills 既可在当前会话执行，也可通过 subagents 在隔离上下文执行。
...
| **[Agent teams](/en/agent-teams)** | 协调多个独立 Claude Code 会话 | 并行研究、新功能开发、竞争性假设调试 | 同时拉起 reviewer 检查安全、性能和测试 |
| **MCP** | 连接外部服务 | 外部数据或外部动作 | 查询数据库、发 Slack、控制浏览器 |
| **Hook** | 在事件上触发的确定性脚本 | 可预测自动化，不涉及 LLM | 每次文件编辑后运行 ESLint |

**[Plugins](/en/plugins)** 是打包层。plugin 将 skills、hooks、subagents 和 MCP servers 打包成一个可安装单元。plugin skill 具备命名空间（例如 `/my-plugin:review`），因此多个 plugins 可共存。当你希望跨多个仓库复用同一套配置，或通过 **[marketplace](/en/plugin-marketplaces)** 分发给他人时，使用 plugins。
...
**两者可组合。** subagent 可预加载特定 skills（`skills:` 字段）。skill 也可用 `context: fork` 在隔离上下文运行。详见 [Skills](/en/skills)。
...
**经验法则：** CLAUDE.md 保持在 200 行以内。若持续增长，把参考内容迁到 skills，或拆分到 [`.claude/rules/`](/en/memory#organize-rules-with-clauderules)。
...
**Rules** 用于让 CLAUDE.md 保持聚焦。带有 [`paths` frontmatter](/en/memory#path-specific-rules) 的 rules 仅在 Claude 处理匹配文件时加载，从而节省上下文。
...
> Agent teams 属于实验功能，默认关闭。配置与限制见 [agent teams](/en/agent-teams)。
...
- **CLAUDE.md 文件**是叠加关系：各层内容会同时进入 Claude 上下文。启动时加载工作目录及其上级文件；在子目录工作时动态加载子目录文件。指令冲突时，Claude 会进行协调，通常更具体的指令优先。详见 [how CLAUDE.md files load](/en/memory#how-claudemd-files-load)。
- **Skills 与 subagents**按名称覆盖：同名时按优先级胜出（skills：managed > user > project；subagents：managed > CLI flag > project > user > plugin）。plugin skills 使用[命名空间](/en/plugins#add-skills-to-your-plugin)避免冲突。详见 [skill discovery](/en/skills#where-skills-live) 与 [subagent scope](/en/sub-agents#choose-the-subagent-scope)。
- **MCP servers**按名称覆盖：local > project > user。详见 [MCP scope](/en/mcp#scope-hierarchy-and-precedence)。
- **Hooks**合并：所有已注册 hooks 都会在匹配事件上触发，不区分来源。详见 [hooks](/en/hooks)。
...
**继承：** Claude 会从工作目录一路向上读取 CLAUDE.md，到根目录为止；访问子目录文件时也会发现并加载嵌套文件。详见 [How CLAUDE.md files load](/en/memory#how-claudemd-files-load)。

建议：CLAUDE.md 保持在约 500 行以内。参考资料应迁移到按需加载的 skills。

#### Skills

Skills 是 Claude 工具箱中的扩展能力。它们可以是参考材料（如 API 风格指南），也可以是通过 `/<name>` 触发的工作流（如 `/deploy`）。Claude Code 自带 [bundled skills](/en/skills#bundled-skills)，例如 `/simplify`、`/batch`、`/debug`，开箱即用。你也可以创建自定义 skills。Claude 会在适当时使用 skills，或由你直接调用。
...
**Context 成本：** 默认启用的 [Tool search](/en/mcp#scale-with-mcp-tool-search) 会将 MCP tools 控制在最多 10% context，其他按需延迟加载。
...
**时机：** 触发时。hooks 在特定生命周期事件触发，如 tool 执行、会话边界、prompt 提交、权限请求与 compaction。完整列表见 [Hooks](/en/hooks)。
...
- [CLAUDE.md](/en/memory)：存储项目上下文、约定与指令
- [Skills](/en/skills)：为 Claude 提供领域知识与可复用工作流
- [Subagents](/en/sub-agents)：将工作卸载到隔离上下文
- [Agent teams](/en/agent-teams)：协调多个并行会话
- [MCP](/en/mcp)：将 Claude 连接到外部服务
- [Hooks](/en/hooks-guide)：用 hooks 自动化工作流
- [Plugins](/en/plugins)：打包并共享功能集合
- [Marketplaces](/en/plugin-marketplaces)：托管并分发 plugin 集合

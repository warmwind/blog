...
本指南介绍核心架构、内置能力，以及[高效使用技巧](#高效使用-claude-code)。如果你想看按步骤讲解，请看 [Common workflows](https://code.claude.com/docs/en/common-workflows)。如果你关注 skills、MCP、hooks 等扩展能力，请看 [Extend Claude Code](https://code.claude.com/docs/en/features-overview)。
...
你可以使用[多个模型](https://code.claude.com/docs/en/model-config)，它们有不同取舍。Sonnet 适合大多数编码任务；Opus 在复杂架构决策中推理更强。你可以在会话中用 `/model` 切换，或用 `claude --model <name>` 指定启动模型。
...
| **代码智能** | 编辑后查看类型错误与告警、跳转定义、查找引用（需[代码智能插件](https://code.claude.com/docs/en/discover-plugins#code-intelligence)） |

以上是主要能力。Claude 还包含用于启动 subagents、向你提问及其他编排任务的 tools。完整列表见 [Tools available to Claude](https://code.claude.com/docs/en/tools-reference)。
...
**扩展基础能力：** 内置 tools 是基础。你可以用 [skills](https://code.claude.com/docs/en/skills) 扩展 Claude 的能力，用 [MCP](https://code.claude.com/docs/en/mcp) 连接外部服务，用 [hooks](https://code.claude.com/docs/en/hooks) 自动化工作流，并把任务分派给 [subagents](https://code.claude.com/docs/en/sub-agents)。这些扩展都建立在核心 agentic loop 之上。关于如何为你的需求选择扩展方式，见 [Extend Claude Code](https://code.claude.com/docs/en/features-overview)。

## Claude 可以访问什么

本指南重点讲终端。Claude Code 也可运行在 [VS Code](https://code.claude.com/docs/en/vs-code)、[JetBrains IDEs](https://code.claude.com/docs/en/jetbrains) 以及其他环境中。
...
* **你的 [CLAUDE.md](https://code.claude.com/docs/en/memory)。** 你用来存储项目级说明、约定和上下文的 markdown 文件，Claude 每次会话都会读取。
* **[Auto memory](https://code.claude.com/docs/en/memory#auto-memory)。** Claude 在工作时自动保存的学习信息，例如项目模式和你的偏好。每次会话开始时会加载 MEMORY.md 的前 200 行。
* **你配置的扩展。** 用于外部服务的 [MCP servers](https://code.claude.com/docs/en/mcp)、用于流程的 [skills](https://code.claude.com/docs/en/skills)、用于委派工作的 [subagents](https://code.claude.com/docs/en/sub-agents)、用于浏览器交互的 [Claude in Chrome](https://code.claude.com/docs/en/chrome)。
...
**sessions 彼此独立。** 每个新 session 都从全新上下文窗口开始，不带前一个 session 的对话历史。Claude 可以通过 [auto memory](https://code.claude.com/docs/en/memory#auto-memory) 跨 session 持久化学习信息，你也可以在 [CLAUDE.md](https://code.claude.com/docs/en/memory) 里加入自己的持久化指令。
...
由于 session 与目录绑定，你可以用 [git worktrees](https://code.claude.com/docs/en/common-workflows#run-parallel-claude-code-sessions-with-git-worktrees) 为不同分支创建不同目录，从而并行运行多个 Claude sessions。
...
Claude 的 context window 包含对话历史、文件内容、命令输出、[CLAUDE.md](https://code.claude.com/docs/en/memory)、已加载 skills 以及系统指令。随着工作推进，上下文会逐渐填满。Claude 会自动 compact，但对话早期的指令可能丢失。持久规则应写在 CLAUDE.md 中，可运行 `/context` 查看上下文占用。
...
[Skills](https://code.claude.com/docs/en/skills) 是按需加载。session 开始时 Claude 只看到 skill 描述，完整内容仅在使用 skill 时加载。对于你手动触发的 skills，可设置 `disable-model-invocation: true`，让描述在需要前不进入上下文。

[Subagents](https://code.claude.com/docs/en/sub-agents) 拥有与主会话完全隔离的独立上下文窗口。它们的工作不会膨胀你的主上下文。完成后仅返回摘要。这种隔离正是 subagents 在长会话中有价值的原因。

关于各特性的上下文开销见 [context costs](https://code.claude.com/docs/en/features-overview#understand-context-costs)，关于降低 token 使用见 [reduce token usage](https://code.claude.com/docs/en/costs#reduce-token-usage)。
...
你也可以在 `.claude/settings.json` 中允许特定命令，避免每次都询问。例如 `npm test` 或 `git status` 这类可信命令。设置可以从组织级策略到个人偏好逐层生效。详见 [Permissions](https://code.claude.com/docs/en/permissions)。

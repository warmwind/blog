...
最新模型经过训练，可分析先前会话状态并生成 compaction item，以加密且 token-efficient 的表示保留关键历史状态。完成 compaction 后，下一个 context window 由该 compaction item 与上一窗口中的高价值部分组成。这样在长时、多步骤、tool-driven 会话中，即便跨越窗口边界，workflow 仍可保持连贯。[Codex 依赖该机制](/index/unrolling-the-codex-agent-loop/) 来支撑长时间编码任务与迭代式工具执行而不降低质量。
